...
output_file_path = 'C:/Users/tibor/Desktop/FREEBASE/people_person_second_gen'
    ...
    if is_person(temp_object_data):
        ...
        if name is not None:
            ...
            if birth_date is not None:
                ...
                writer.add_document(name=str(name), birthdate=str(birth_date), deathdate=str(get_death_date(temp_object_data)))
                # fileobject.close()
                return True
    return False


def get_people_objects(writer):
    temp_object_data = []
    ...
    with open(open_this_file, encoding="utf-8") as fileobject:
        for line in fileobject:
            ...
            if fetched_object_id != prev_object_id:
                if process_temp_object_for_get_people_objects(temp_object_data, writer):  # if it is person & is written to file
                    person_counter += 1
